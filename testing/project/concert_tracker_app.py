            ...
            if this_concert:
                if concert_type == "Rock":
                    for musician_ob in musicians_of_the_group:
                        if "play the drums with drumsticks" in musician_ob.skills \
                                or "sing high pitch notes" in musician_ob.skills \
                                or "play rock" in musician_ob.skills:
                            pass
                        else:
                    ...
                    for musician_ob in musicians_of_the_group:
                        if "play the drums with drumsticks" in musician_ob.skills \
                                or "sing low pitch notes" in musician_ob.skills \
                                or "play metal" in musician_ob.skills:
                            pass
                        else:
                            raise Exception(f"The {band_name} band is not ready to play at the concert!")
                    break
                    ...
                    for musician_ob in musicians_of_the_group:
                        if "play the drums with drum brushes" in musician_ob.skills \
                                or "sing high pitch notes" in musician_ob.skills \
                                or "sing low pitch notes" in musician_ob.skills \
                                or "play jazz" in musician_ob.skills:
                            pass
                        else:
                            raise Exception(f"The {band_name} band is not ready to play at the concert!")

        concert_profit = (concert_obj.audience * concert_obj.ticket_price) - concert_obj.expenses
        return f"{band_name} gained {concert_profit:.2f}$ from the {concert_obj.genre} concert in {concert_place}."
